...
//		out.print(action);
		...
		if("addEmp".equals(action)) {
			...
			if(!errMsg.isEmpty()) {
				request.setAttribute("isSucess", "密碼或姓名格式不正確，請重新新增!");
...
//			request.getRequestDispatcher("/back_end/emp/empAcc.jsp").forward(request, response);
			...
			request.getSession().setAttribute("isSucess", "員工:" + emp_name + "新增成功!");
		...
		if("updateEmp".equals(action)) {
			...
			request.getSession().setAttribute("isSucess", "員工編號:" + emp_no + "，員工姓名:" + emp_name + "，更新成功!");
			response.sendRedirect(request.getContextPath() + "/back_end/emp/empAcc.jsp");
//			request.getRequestDispatcher("/back_end/emp/empAcc.jsp").forward(request, response);
		}
