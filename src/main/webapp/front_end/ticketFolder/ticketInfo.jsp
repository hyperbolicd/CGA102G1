...
<body>
	...
	<div class="ord_info" style="padding: 10px">
		<div class="ord_detail" style="display: flex">
			<div class="card">
				<div class="card-body" style="font-weight: bolder;">電影名稱:
					${movieVO.mvName}</div>
			</div>
			<div class="card" style="margin-left: 10px; font-weight: bolder">
				<div class="card-body">上映廳院: ${hallVO.hlName}</div>
			</div>
			<div class="card" style="margin-left: 10px; font-weight: bolder">
				<div class="card-body">
					場次時間:
					<fmt:formatDate pattern="yyyy年MM月dd日 H點m分"
						value="${showSeatVO.SH_TIME}" />
				</div>
			</div>
		</div>
		<div class="ord_content" style="display: flex">
			<div class="ticket_container" style="width: 70%; margin-top: 10px">
				<div class="ticket_table">
					<div class="card">
						<div class="card-header" style="font-weight: bold;">票卷資訊</div>
						<div class="card-body">

							<input type="hidden" id="tkOrdID" value="${tkOrdID}">

							<table id="tableAjax" class="table  table-hover"
								style="width: 100%">
								<thead>
									<tr>
										<th>票種</th>
								...
								<tbody>
									<c:forEach items="${ordDtList}" var="ordDt" varStatus="loop">
										<tr>
											...
											<td><c:if test="${ordDt.state==0}">
                    		已付款
                    		</c:if> <c:if test="${ordDt.state==1}">
                    		已入場
                    		</c:if> <c:if test="${ordDt.state==2}">
                    		已退票
                    		</c:if></td>
											<td><c:if test="${ordDt.state==0}">
													<button class="btn btn-dark" id="${ordDt.tkDtID}"
														onclick="refund(this)">退票</button>
												</c:if> <input type="hidden" id="seatIndex${ordDt.tkDtID}"
												value="${ordDt.seat}"></td>
										</tr>
									</c:forEach>
								</tbody>
							...
							<div class="ticket_btn">
								※請入場時出示畫面給現場人員驗票 <br> ※購買學生票和敬老票請於入場時出示相關證件
								<button type="button" id="tkChecking" onclick="tkChecking()"
									class="btn btn-success btn-lg btn-block" style="width: 30%">驗票</button>
							</div>
			...
			<div class="food_container"
				style="width: 30%; margin: 10px 0px 0px 5px">
				<div class="food_table">
					<div class="card">
						<div class="card-header" style="font-weight: bold;">餐飲資訊</div>
						<div class="card-body">
							<table id="tableAjax" class="table  table-hover"
								style="width: 100%">
								<thead>
								...
								<tbody>
									...
									<c:forEach items="${foodOrdList}" var="foodOrdDt"
										varStatus="loop">
										...
										<tr>
											...
											<td><c:if test="${foodOrdDt.fdState==0}">
                    		已付款
                    		</c:if> <c:if test="${foodOrdDt.fdState==1}">
                    		已取餐
                    		</c:if> <c:if test="${foodOrdDt.fdState==2}">
                    		已退餐
                    		</c:if></td>
										</tr>
									</c:forEach>
            	 ...
            	 <div class="food_btn"
									...
									<c:forEach items="${foodOrdList}" var="foodOrdDt"
										...
										<c:if test="${foodOrdDt.fdState==0}">
											...
											<button type="button" id="takeMeal" onclick="takeMeal()"
												class="btn btn-success btn-lg btn-block" style="width: 30%">兌換餐點</button>
											<button type="button" id="cancelMeal" onclick="cancelMeal()"
												class="btn btn-danger" id="">退餐</button>
					...
					<table id="tableAjax" class="table  table-hover"
						...
						<tbody>
                    		...
                    		已退票
									...
									<td><c:if test="${ordDt.state==0}">
											<button class="btn btn-success" id="getin${ordDt.tkDtID}"
												onclick="admission(this)">入場</button>
										</c:if> <input type="hidden" id="tkDtId_getin${ordDt.tkDtID}"
										value="${ordDt.tkDtID}"></td>
								</tr>
							</c:forEach>
						</tbody>
					</table>
...
<script
	src="${pageContext.request.contextPath}/front_end/ticketFolder/ticketInfo.js"></script>
