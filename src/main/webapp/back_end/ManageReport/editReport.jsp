...
<!-- ****************************************************** -->
<link rel="stylesheet"
	href="${pageContext.request.contextPath}/back_end/ManageReport/editReport.css">
<link rel="stylesheet"
...
<body>
	...
	<main>
		<div id="main" class="mainBox">
			...
			<form METHOD="post"
				action="${pageContext.request.contextPath}/ReportServlet.do">
				<div class="custom-control custom-switch" style="margin-left: 10px">
					<input type="checkbox" class="custom-control-input"
						id="updateSameRP" name="updateSameRP"> <label
						class="custom-control-label" for="updateSameRP">一次處理同則評論</label>
				</div>
				...
				<div class="statusBox">
					...
					<div class="selectBox">
						<label for="">選擇處理方式:</label> <select class="custom-select"
							name="changeCmState" id="selectState">
							<c:forEach var="item" items="${changeCM_STATE}" varStatus="index">
								<option value="${index.index}"
									${index.index == cmtVO.CM_STATE ? 'selected="selected"' : ''}>${item}</option>
							</c:forEach>
						</select>
					</div>
				</div>
				...
				<div class="badBox">
					<div class="pic">
						<img
							src="${pageContext.request.contextPath}${cmtMemberVO.member_Pic}"
							class="rounded-circle" alt="">
						<p>評論會員:${cmtMemberVO.member_Name}</p>
					</div>
					<div class="commInfo">
						<div class="speech-bubble" style="word-break: break-all">
							<h5 id="commText">${cmtVO.CM_TEXT}</h5>
						</div>
					</div>
					<hr size="8px" align="center" width="100%">
					...
					<div class="pic">
						<img
							src="${pageContext.request.contextPath}${rpMemberVO.member_Pic}"
							class="rounded-circle" alt="">
				...
				<div class="editBox">
					...
					<button type="button" onclick="history.back()"
						class="btn btn-secondary btn-lg">取消</button>
