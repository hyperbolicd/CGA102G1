...
<!-- ***************************自己的CSS****************************** -->
<link rel="stylesheet"
	href="${pageContext.request.contextPath}/back_end/ManageReport/report_main.css">
<link rel="stylesheet"
	...
	<main>
		<div id="main">
				...
				<c:if test="${undealCount > 0}">
					<div class="alertbox">
						<div
							...
							<i
								class="start-icon fa fa-exclamation-triangle faa-flash animated"></i>
							<strong class="font__weight-semibold">請注意!</strong>您有<strong
								class="font__weight-semibold">${undealCount}</strong>則檢舉尚未處理 !
						</div>
					</div>
				</c:if>
				<c:if test="${undealCount == 0}">
			...
			<div class="tablebox" style="border: solid 0px red;">
				<table id="maintable"
					class="table table-hover table-bordered single-ellipsis">
					<thead>
						<tr>
							<th>檢舉單號</th>
							<th>評論編號</th>
					...
					<tbody>
						...
						<c:forEach var="reportVO" items="${reportList}"
							...
							<tr>
								...
								<td><fmt:formatDate value="${reportVO.rpDate}"
								...
								<td>
									<form METHOD="post"
										...
										<input type="hidden" name="memberId"
											value="${reportVO.memberId}"> <input type="submit"
											value="管理檢舉" class="btn btn-secondary">
									</form>
								</td>
							</tr>
						</c:forEach>
